...
import { UpdateProfileSchema } from '@srm-recollab/shared-utils';
import { UserRole } from '@srm-recollab/types';
import { 
  BookOpen, 
  MapPin, 
  FileText, 
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto select-none">
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 relative overflow-hidden flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left bg-white dark:bg-slate-900/15 shadow-sm transition-colors duration-300">
        <div className="absolute inset-0 bg-gradient-to-r from-srm-crimson/[0.01] to-srm-gold/[0.01] -z-10" />
        
        {/* Avatar Image */}
        <div className="relative shrink-0">
          <img 
            src={currentUser?.image || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=180'} 
            className="w-20 h-20 rounded-full border-2 border-srm-gold object-cover shadow-md"
          />
          <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-slate-900 dark:bg-slate-950 border border-slate-800/80 flex items-center justify-center text-slate-400 shadow">
            <Sparkles className="w-3.5 h-3.5 text-srm-gold animate-pulse" />
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          {isEditing ? (
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/15 space-y-6 text-left shadow-sm">
              <span className="text-[9px] font-black text-srm-crimson dark:text-srm-gold uppercase tracking-widest block border-b border-slate-100 dark:border-slate-850 pb-2">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <option value="FACULTY" className="bg-slate-50 dark:bg-slate-950">SRM Faculty</option>
                  </select>
                ...
                <div className="space-y-2">
                  <label className="block text-[10px] font-black text-slate-450 dark:text-slate-500 uppercase tracking-widest">SRM Department</label>
                  <select
                    {...register('department')}
                    className="w-full px-3.5 py-2.5 rounded-xl glass-input text-xs font-semibold cursor-pointer text-slate-700 dark:text-slate-350"
                  >
            ...
            <div className="space-y-6">
              ...
              <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/15 text-left shadow-sm space-y-4">
                <h3 className="font-display font-extrabold text-sm uppercase tracking-wider text-slate-900 dark:text-white flex items-center gap-2 border-b border-slate-100 dark:border-slate-850 pb-2">
                  <FileText className="w-4.5 h-4.5 text-srm-crimson dark:text-srm-gold" />
              ...
              <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/15 text-left shadow-sm space-y-4">
                <h3 className="font-display font-extrabold text-sm uppercase tracking-wider text-slate-900 dark:text-white flex items-center gap-2 border-b border-slate-100 dark:border-slate-850 pb-2">
                  <Award className="w-4.5 h-4.5 text-srm-crimson dark:text-srm-gold" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-850">
                    ...
                    <p className="text-xs text-slate-800 dark:text-white font-bold mt-1.5 flex items-center gap-1.5">
                      <MapPin className="w-4 h-4 text-srm-crimson shrink-0" />
                  ...
                  <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-850">
                    ...
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 font-black mt-1.5 flex items-center gap-1">
                      <Check className="w-4 h-4 shrink-0" />
                      <span>Verified SRMist Intranet Node</span>
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/15 text-left shadow-sm space-y-4">
                <h3 className="font-display font-extrabold text-sm uppercase tracking-wider text-slate-900 dark:text-white flex items-center gap-2 border-b border-slate-100 dark:border-slate-850 pb-2">
                  <BookOpen className="w-4.5 h-4.5 text-srm-crimson dark:text-srm-gold" />
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-3.5">
                    <div className="p-2 bg-srm-crimson/5 dark:bg-srm-gold/5 border border-srm-crimson/15 dark:border-srm-gold/15 rounded text-srm-crimson dark:text-srm-gold font-black text-[9px] uppercase tracking-wider shrink-0 mt-0.5">2026</div>
                    <div>
                  ...
                  {userThreads.map(t => (
                    <div key={t.id} className="flex items-start gap-3.5 border-t border-slate-100 dark:border-slate-850/50 pt-3">
                      <div className="p-2 bg-srm-blue/10 border border-srm-blue/20 rounded text-srm-blue font-black text-[9px] uppercase tracking-wider shrink-0 mt-0.5">Proposal</div>
                      <div>
                        <Link href={`/threads/${t.id}`} className="text-xs font-bold text-slate-800 dark:text-slate-200 leading-snug hover:underline flex items-center gap-1">
                          <span>{t.title}</span>
                          <Sparkles className="w-3 h-3 text-srm-gold" />
                        </Link>
              ...
              <div className="flex flex-wrap gap-1.5">
                {selectedInterests.length === 0 ? (
                  ...
                  selectedInterests.map((interest) => (
                    <span
                      key={interest}
                      className="inline-flex items-center px-2.5 py-1.5 rounded-lg text-[10px] font-bold bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-srm-gold"
                    >
                      <span>{interest}</span>
                      <button
                        type="button"
